...
func TestDNSZoneNotVerifiedError_Error(t *testing.T) {
	type fields struct {
		parent      error
		code        string
		description string
	...
	tests := []struct {
	...
	}{
		{
			name: "nil detail",
			fields: fields{
				parent: ErrDNSZoneNotVerified,
				code:   "dns_zone_not_verified",
				description: "The DNS zone could not be verified, check the " +
					"nameservers a set correctly",
			},
			want: "katapult: unprocessable_entity: dns_zone_not_verified: " +
				"The DNS zone could not be verified, check the nameservers " +
				"a set correctly",
		},
		{
			name: "empty detail",
			...
			fields: fields{
				...
				detail: &DNSZoneNotVerifiedErrorDetail{
					VerificationDetails: struct {
						...
						Nameservers: []string{"ns1.foo.io", "ns2.bar.io"},
					},
				},
			},
			want: "katapult: unprocessable_entity: dns_zone_not_verified: " +
				"The DNS zone could not be verified, check the nameservers " +
				"a set correctly -- " +
				"nameservers: ns1.foo.io, ns2.bar.io",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			theError := NewDNSZoneNotVerifiedError(
